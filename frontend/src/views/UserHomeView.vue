...
watch(result, (value) => {
  if (value) resultsExpanded.value = true;
});
...
<template>
  <section class="route-planning-layout" aria-labelledby="user-title">
    <aside class="route-panel route-control-panel" :class="{ 'is-collapsed': !controlExpanded }">
      <button
        class="mobile-sheet-toggle"
        type="button"
        :aria-expanded="controlExpanded"
        aria-controls="route-control-content"
        @click="controlExpanded = !controlExpanded"
      >
        <span><AppIcon name="route" />路线设置</span>
        <span>{{ controlExpanded ? '收起' : '展开' }}</span>
      </button>
      <div id="route-control-content" class="mobile-sheet-content">
    ...
    <aside
      class="route-results-panel"
      :class="{ 'is-collapsed': !resultsExpanded }"
      aria-label="路线规划结果"
    >
      <button
        class="mobile-sheet-toggle"
        ...
        aria-controls="route-results-content"
        @click="resultsExpanded = !resultsExpanded"
      >
        <span><AppIcon name="services" />路线结果</span>
        <span>{{ resultsExpanded ? '收起' : '展开' }}</span>
      </button>
      <div id="route-results-content" class="mobile-sheet-content">
        <template v-if="result">
          ...
          <section v-if="selectedRoute" class="route-details" aria-live="polite">
            ...
            <p v-else class="safe-note"><AppIcon name="governance" />当前路线没有额外警告</p>
              ...
              <dl>
                ...
                <div>
                  ...
                  <dd>
                    {{
                      (
                        selectedRoute.costBreakdown.barrier +
                        selectedRoute.costBreakdown.uncertainty
                      ).toFixed(1)
                    }}
                  </dd>
                </div>
        ...
        <div v-else class="results-empty">
          <span class="empty-route-symbol" aria-hidden="true"
