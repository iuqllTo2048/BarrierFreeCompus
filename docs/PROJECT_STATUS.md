...
- 状态：路线助手、受控 Tool Calling、途经点、多候选路线和设施删除均已实现并验收，功能提交已推送 `origin/main`；2026-09-02 已完成仓库冗余文档与事实清理；2026-09-18 将途经点上限调整为 8 个
- Git Tag：`v1.0`（用户已明确确认创建）

## 2026-09-18 途经点上限调整

- 受控路线 Tool 与后端行程服务统一支持最多 8 个有序途经点，即最多 9 段；第 9 个途经点会在工具或行程服务边界被拒绝。
- 只调整上限和对应提示词、说明、测试；A*、Yen Top-K、路线候选数、SSE 格式和数据库结构不变。
- 自动测试覆盖 8 个途经点的 9 段合并，以及 9 个途经点的拒绝；尚未对真实 DeepSeek 的 8 途经点完整回复时间做承诺。
