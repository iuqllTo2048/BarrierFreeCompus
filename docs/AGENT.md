...
| `calculateAccessibleRoutes` | 一次接收起点、最多 8 个有序途经点、终点、行动方式和偏好；后端逐段调用真实 A* 并合并完整折线、指标、确定性比较和沿途设施，模型不接触节点 ID |
...
含途经点的请求只允许一次 `calculateAccessibleRoutes` 调用。模型负责按用户原话提取顺序，后端负责地点唯一性、最多 8 个途经点、路网连接和重复节点校验，并按“起点 → 途经点 1 → … → 终点”复用单段 A*。只要任一段不可达，完整行程即判定不可达；所有段可达时，服务端合并相邻坐标并汇总距离、时间、楼梯、坡度、风险、设施、障碍和算法指标，前端仍消费原有 `RoutePlanResponse`。
